  ...
  ruby_block "schema_import_ldif" do
    block do
      File.open(import_file, "w") do |f|
        node.nmd_openldap.additional_schemas.each do |schema|
...
# Create a link under node.nmd_openldap.tls.cert_file which points to the server certificate under
...
define :server_certificate_link do
  link node.nmd_openldap.tls.cert_file do
    to "/etc/pki/tls/certs/#{node['fqdn']}.pem"
  end
end

# Create a link under node.nmd_openldap.tls.cacert_path which points to the CA Certificate under
...
define :ca_certificate_link do
  ...
  directory node.nmd_openldap.tls.cacert_path do
    mode 0700
    owner "ldap"
    group "ldap"
  end
  ...
  ruby_block "ca_certificate_link" do
    block do
      ...
      link_name = File.join(node.nmd_openldap.tls.cacert_path, `openssl x509 -hash -noout -in #{ca_cert}`.chomp + ".0")
...
# Create a link under node.nmd_openldap.tls.key_file which points to the private key file under
...
define :private_key_link do
  ...
  link node.nmd_openldap.tls.key_file do
    to "/etc/pki/tls/private/#{node['fqdn']}.key"
    link_type :hard
  end

  file node.nmd_openldap.tls.key_file do
    owner "ldap"
    group "ldap"
    mode  0600
  end
